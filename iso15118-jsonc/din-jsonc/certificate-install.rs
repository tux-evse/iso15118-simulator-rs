...
impl IsoToJson for CertificateInstallResponse {
    fn to_jsonc(&self) -> Result<JsoncObj, AfbError> {
        ...
        jsonc.add("contract_id", self.get_contract_id()?)?;
        jsonc.add("contract_chain", self.get_contract_chain().to_jsonc()?)?;

        let mut encode = String::new();
        general_purpose::STANDARD.encode_string(self.get_public_dh(), &mut encode);
        jsonc.add("public_dh", &encode)?;

        let mut encode = String::new();
        general_purpose::STANDARD.encode_string(self.get_contract_signature(), &mut encode);
        jsonc.add("contract_signature", &encode)?;
        Ok(jsonc)
    }
    fn from_jsonc(jsonc: JsoncObj) -> Result<Box<Self>, AfbError> {
        let rcode = ResponseCode::from_label(jsonc.get("rcode")?)?;
        let id_ref = jsonc.get("id")?;
        let contract_id = jsonc.get("contract_id")?;
        let contract_chain = CertificateChainType::from_jsonc(jsonc.get("contract_chain")?)?;

        let base64 = jsonc.get::<&str>("public_dh")?;
        let public_dh = match general_purpose::STANDARD.decode(base64) {
            Ok(value) => value,
            Err(_) => {
                return afb_error!(
                    "certificate-install-req-from-jsonc",
                    "fail to decode base64 public_dh"
                )
            }
        };
